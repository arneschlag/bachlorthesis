...
mode = "val"
with open(f'/mnt/data/bdd100/bdd100k/labels/bdd100k_labels_images_{mode}.json', 'r') as data_file:
    json_data = data_file.read()

with open(f'/mnt/data/bdd100/bdd100k/labels/pose_21/pose_{mode}.json', 'r') as data_file:
...
def gen_anno(label, f_id, anno_id):
...
categories = []
import glob
path = f'/mnt/data/bdd100/bdd100k/images/100k/{mode}/'
valid_imges = glob.glob(f'{path}*')
for f_id, frame in enumerate(pose_data['frames']):
    extra = data[lookup_extra_data[frame['name']]]
    # check if available
    if path+frame['name'] in valid_imges:
        loc_images.append(dict(file_name=f'{mode}/{frame["name"]}',
                            height=720,
                            width=1280,
                            id=f_id,
        ...
        if frame['labels']:
            for label in frame['labels']:
                ...
                anno = gen_anno(label, f_id, anno_id)
                if anno:
...
import json
with open(f'bdd100k_{mode}_with_all.json', 'w', encoding ='utf8') as json_file:
