...
def find_semseg_img(fname, person_idx):
    folders = glob.glob('/mnt/data/AGORA/validation/*')
...
def calculate(row, image_id, annotation_id):
    ...
    if img_path not in image_dir:
        ...
        size = get_image_size(f'/mnt/data/AGORA/imgs/validation/{img_path}')
        fname = img_path.split('.')[0]
        # if semseg_img:
        #    bboxs_real = calculate_bbox(semseg_img)
        if size:
            loc_images.append(dict(file_name=f'imgs/validation/{img_path}',
                            height=size[1],
                            width=size[0],
                            id=image_id)
            )
...
for subset in tqdm(val_dataset):
    df = pd.read_pickle(f'/mnt/data/AGORA/validation_SMPL/SMPL/{subset}_withjv.pkl')
...
print("Export")
with open(f'agora_validation_all_n.json', 'w', encoding ='utf8') as json_file:
